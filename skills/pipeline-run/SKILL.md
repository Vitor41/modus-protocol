...
2. Toda nova execução invalida conclusões anteriores sobre elegibilidade, aprovação e bloqueio. Execute `../../runtime/pipeline.ps1 trello --action snapshot --project-root <raiz> --output .pipeline/tmp/tracker-snapshot.json` já com acesso externo autorizado/elevado no launcher oficial. O cliente faz até três tentativas totais somente nas leituras idempotentes; não improvise outra integração nem repita manualmente além desse orçamento. O adaptador consulta somente o board declarado e relê todos os cards acionáveis, comentários, movimentações e anexos atuais. Os sinais são derivados apenas dos eventos posteriores à entrada na lista atual; um evento de fase anterior não pode bloquear a fase presente. Em `refinement`, preserve cards ainda sem chave ou labels: o PO é responsável por normalizá-los.
...
Leia `references/execution-protocol.md`, `references/tracker-comment-protocol.md` e `../../docs/AUTONOMY_POLICY.md` antes de qualquer escrita, retomada ou transição.
...
8. Aceite um handoff apenas com saída, recibo de execução e evidência exigidos pelo gate. Se o role gate falhar, execute `recovery.actions` antes de classificar impedimento. `changes_required` do Review e `rejected` do QA são retornos técnicos normais: acione DEV imediatamente na mesma lane e no mesmo loop.
...
13. `requires_human: true` exige um `blocker.kind` permitido por `AUTONOMY_POLICY.md`: regra de negócio material, aprovações previstas, limite de retorno, expansão estrutural de escopo, risco sistêmico ou autorização externa ausente. Falha técnica local, gate mecânico, teste, conflito, evidência ou artefato corrigível nunca usa espera humana. Cada impedimento afeta somente o escopo declarado: `card`, `delivery_group` ou `dependency_group`; registre-o e continue cards e grupos independentes.
...
15. Depois de lançar qualquer agente, permaneça na mesma execução e use a espera disponível até obter seu resultado terminal. Comentário intermediário pode informar que o papel está rodando, mas é proibido enviar resposta final, devolver o controle ao usuário ou afirmar que o agente “continuará sozinho”. Ao concluir Review, consuma o handoff, mova para QA e lance QA; ao concluir QA, processe aprovação ou retorno ao DEV e replaneje a fila técnica, inclusive o próximo card que estava sob WIP.
16. Antes da resposta final, materialize `.pipeline/tmp/<RUN_ID>-close-receipt.json` com todos os jobs lançados e o plano obtido de um snapshot posterior ao último evento de agente. Execute `../../runtime/pipeline.ps1 run-close-gate --receipt <recibo> --format json`. Só finalize com `PASS / FINAL_RESPONSE_GRANTED`; em `FAIL / CONTINUE_RUN`, execute todas as `actions` retornadas e repita o gate. O chat não substitui esse gate.
...
- Respeite os orçamentos de recuperação do plano. Esgotá-los isola a falha técnica; não cria autorização para fallback, integração alternativa ou repetição cega de escrita.
