...
11. Após um handoff e uma transição confirmados, gere novo snapshot e replaneje com `--continue-run-id <RUN_ID atual>`; confira que a saída contém o mesmo `run_id` e `continuing: true` antes de acionar o próximo papel. A lista atual e suas evidências frescas são a autoridade: lock, cápsula ou execução ativa de uma lista/papel anterior são reconciliados como continuidade e não bloqueiam a nova fase. Repita o ciclo PO → UX/UI → DEV → Code Review → QA enquanto houver trabalho independente elegível.
12. Os únicos gates humanos canônicos são: regra de negócio material ausente, `Tela aprovada`, validação da atividade (`APROVADO PARA PRD`) e limite de retorno. Cada um afeta somente o escopo declarado: `card`, `delivery_group` ou `dependency_group`. Lock externo atual ou falha técnica comprovada também isolam somente o escopo afetado; não os apresente como decisão humana. Registre e adie esse escopo e continue cards e grupos independentes.
13. Após qualquer gate humano ou bloqueio localizado, gere obrigatoriamente um novo snapshot e plano antes de responder. A resposta final deve classificar todos os cards acionáveis somente a partir desse plano final e citar a cobertura confirmada. Encerre a automação somente quando esse replanejamento retornar fila elegível esgotada ou uma falha global de contrato, ferramenta, releitura ou segurança impedir o processamento. Um handoff `PASS` ou o próximo gate de um único card nunca é condição de parada.
