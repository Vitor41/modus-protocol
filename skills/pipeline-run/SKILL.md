...
13. Após qualquer gate humano ou bloqueio localizado, gere obrigatoriamente um novo snapshot e plano antes de responder. Gates isolados retiram apenas seu card/grupo do slot; PO e UX/UI continuam mesmo quando a lane técnica aguarda humano. A resposta final classifica todos os cards acionáveis pelo plano final. Encerre somente quando não existir slot elegível nem agente ativo, ou uma falha global comprovada impedir todas as lanes. Um handoff `PASS`, um gate de um card, um item adiado ou o término de um agente nunca é condição de parada.
